...
class _GrammarQuestScreenState extends State<GrammarQuestScreen> {
  ...
  Color word1TextColor = Colors.black; 
  Color word2TextColor = Colors.black; 
  Color word3TextColor = Colors.black;
  bool loading = false;
  ...
  void initState() {
    ...
    _startCountdown();
    // Check subscription status after frame is built
  WidgetsBinding.instance.addPostFrameCallback((_) {
   ...
   // Fixed _checkSubscriptionStatus method
void _checkSubscriptionStatus() {
  final state = context.read<GrammarQuestCubit>().state;
  if (state is GrammarQuestLoaded) {
    setState(() {
      _isFreehitUser = state.isFreeHitUser;
      _isSubscribePlan = state.isSubscribePlan;
    });
    
    // Check and show dialog if needed
    _checkAndShowDialog(state);
  }
}

// Fixed _checkAndShowDialog method with proper validation
void _checkAndShowDialog(GrammarQuestState questState) {
  ...
  bool _canAccessNextLevel() {
    // If user is subscribed or not a free user, allow access
    if (!_isFreehitUser || _isSubscribePlan) {
      return true;
    }
    
    // Free users can only proceed if next level is already completed
    if (widget.index + 1 < widget.questions.length) {
      return widget.questions[widget.index + 1].completed;
    }
  ...
  bool get _shouldBlockNavigation => _isFreehitUser && !_isSubscribePlan;
  ...
  void _showCongratulationsPopup() {
    ...
    final bool canAccessNext = _canAccessNextLevel();
                ...
                Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        hasNextLevel && canAccessNext
                            ? "You matched all correctly.\nReady for the next level?"
                            : hasNextLevel && !canAccessNext
                                ? "You matched all correctly! 🎉\nSubscribe to unlock more levels."
                                : "You matched all correctly. 🎉\nGame Completed!",
                        textAlign: TextAlign.center,
                      ...
                      Row(
                                ...
                                Navigator.pushReplacement(
                                  ...
                                  MaterialPageRoute(
                                    builder: (_) => GrammarQuestScreen(
                                      imageUrl: widget.questions[widget.index + 1].imagePath,
                                      questions: widget.questions,
                                      index: widget.index + 1,
                                      level: widget.level + 1,
                                      questId: widget.questions[widget.index + 1].id,
                          ...
                          else if (hasNextLevel && !canAccessNext)
                            ElevatedButton(
                              style: ElevatedButton.styleFrom(
