...
class MainActivity : FlutterActivity() {
    ...
    private val CALL_PERMISSIONS_CHANNEL = "picturo_call_permissions"
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        // Call permissions methods
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CALL_PERMISSIONS_CHANNEL)
                .setMethodCallHandler { call, result ->
                    when (call.method) {
                        "checkAndRequestCallPermissions" -> {
                            checkAndRequestCallPermissions(result)
                        }
                        else -> result.notImplemented()
                    }
                }
    ...
    private fun checkAndRequestCallPermissions(result: MethodChannel.Result) {
        ...
        if (Build.VERSION.SDK_INT >= 34) {
            ...
            if (!notificationManager.canUseFullScreenIntent()) {
                missing.add("full_screen_intent")
                try {
                    val intent = Intent(Settings.ACTION_MANAGE_APP_USE_FULL_SCREEN_INTENT)
                    intent.data = Uri.parse("package:$packageName")
                    startActivity(intent)
                } catch (e: Exception) {
                    log("Failed to open full screen intent settings: ${e.message}")
                }
            }
        }
        ...
        if (!Settings.canDrawOverlays(this)) {
            ...
            try {
                ...
                log("Failed to open overlay settings: ${e.message}")
            }
        }
